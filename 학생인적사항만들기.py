...
# 저장은 Rest API의 기본 형태인 JSON으로 저장 및 불러오기(웹에서 API(자료)의 기본 포맷)
# 함수로 구현
import json # JSON 향식으로 데이터를 저장하고 불러오기 위해 JSON 모듈을 import
...
# 학생 정보 저장
def save_student() :
    with open("student.json",'w',encoding='utf-8') as file : # 들여쓰기를 하고 빠져나오면 close 써야하는데 with는 close 생략 가능(json시 꼭 넣어줘야함)
        json.dump(student_map, file, ensure_ascii=False) # ensure_ascii=False로 인식하지말라고 써줌(json시 꼭 넣어줘야함)
    print("학생 정보가 저장 되었습니다.")

# JSON 파일을 불러 옴
def load_student() : # 어떤 곳이든 불러낼 수 있는 파일
    try:
