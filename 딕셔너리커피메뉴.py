...
while True :
    print("메뉴를 선택하세요 : ")
    menu = input("[1]메뉴 리스트 보기 [2] 메뉴 조회 [3]메뉴 추가 [4]메뉴 삭제 [5]종료 : ") # input은 기본 문자열로 반환 됨
    if menu == "1" :
        for key in menu_dic :
            print(key, ":", menu_dic[key])
    elif menu == "2" :
        rtrv_name = input("조회 할 메뉴를 입력 하세요 : ")
        if rtrv_name in menu_dic : # 딕셔너리에 키가 있는지 확인 하는 조건문
        ...
        if add_menu not in menu_dic :
            ...
            desc = input("제품 설명 : ")
            menu_dic[add_menu] = [category, price, desc] # 추가 시 딕셔너리[키] = 값 새로운 키와 값을 추가
        ...
        if del_menu in menu_dic :
            ...
            for key in menu_dic:  # 메뉴가 삭제 되면 삭제된 메뉴 리스트를 다시 보여 줌
                print(key, ":", menu_dic[key])
